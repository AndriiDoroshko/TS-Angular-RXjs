  ...
  <mat-horizontal-stepper>
    <mat-step
      *ngFor="let step of field.fieldGroup; let index = index; let last = last;">
      <ng-template matStepLabel>{{ step.templateOptions?.label ? step.templateOptions?.label : 'Step'}}</ng-template>
      ...
      <div>
        ...
        <button *ngIf="last"
          [disabled]="!form.valid" mat-raised-button color="primary" class="mt-4"
          type="submit">
          Enter
        </button>
      </div>
